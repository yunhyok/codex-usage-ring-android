...
impl ErrorCode {
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::RateLimitsAuthRequired => "RATE_LIMITS_AUTH_REQUIRED",
            Self::RateLimitsBackend => "RATE_LIMITS_BACKEND",
            Self::RateLimitsHttp401 => "RATE_LIMITS_HTTP_401",
            Self::RateLimitsHttp403 => "RATE_LIMITS_HTTP_403",
            Self::RateLimitsHttp429 => "RATE_LIMITS_HTTP_429",
            Self::RateLimitsHttp4xx => "RATE_LIMITS_HTTP_4XX",
            Self::RateLimitsHttp5xx => "RATE_LIMITS_HTTP_5XX",
            Self::RateLimitsEmpty => "RATE_LIMITS_EMPTY",
            Self::RateLimitsServer => "RATE_LIMITS_SERVER",
            Self::RateLimitsTransport => "RATE_LIMITS_TRANSPORT",
            Self::RateLimitsTimeout => "RATE_LIMITS_TIMEOUT",
            Self::RateLimitsDeserialize => "RATE_LIMITS_DESERIALIZE",
...
fn error_json(method: &str, code: ErrorCode) -> String {
    let message = match code {
        ...
        ErrorCode::RateLimitsAuthRequired => {
            "ChatGPT authentication is required to read rate limits"
        }
        ErrorCode::RateLimitsBackend => "rate limit backend request failed",
        ErrorCode::RateLimitsHttp401 => "rate limit server rejected authentication",
        ErrorCode::RateLimitsHttp403 => "rate limit server denied access",
        ErrorCode::RateLimitsHttp429 => "rate limit server throttled the request",
        ErrorCode::RateLimitsHttp4xx => "rate limit server rejected the request",
        ErrorCode::RateLimitsHttp5xx => "rate limit server failed",
        ErrorCode::RateLimitsEmpty => "rate limit server returned no snapshots",
        ErrorCode::RateLimitsServer => "rate limit server rejected the request",
        ErrorCode::RateLimitsTransport => "rate limit transport failed",
        ErrorCode::RateLimitsTimeout => "rate limit request timed out",
        ErrorCode::RateLimitsDeserialize => "rate limit response was invalid",
