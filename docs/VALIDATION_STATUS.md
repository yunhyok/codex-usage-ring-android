...
| `testMockDebugUnitTest`, `lintMockDebug`, `assembleMockDebug` | Local/CI log and mock APK artifact | **local pass (2026-08-21); CI pending** |
...
| Rust `fmt`, `clippy`, tests | Local/CI log | **local pass: 5/5 tests; CI pending** |
| Native Android gate | Sanitized local report; release requires reviewed `docs/evidence/v0.1.0/native-gate.json` | **NO-GO: upstream runtime not linked/proven** |
| Physical-device install/launch/widget smoke | `docs/evidence/v0.1.0/physical-device.json` plus device metadata | **missing** |
| CycloneDX dependency SBOM and license policy | Gradle `cyclonedxBom` plus `scripts/verify-sbom.ps1` | **local pass: 321 components, 3 exact metadata exceptions; release artifact pending** |
| Cargo dependency license policy | locked `cargo metadata` plus `scripts/verify-cargo-licenses.ps1` | **local pass: 41 locked packages** |
