                ...
                TlsBackend::Rustls => {
                    ...
                    let config_builder = if !config.certs_verification {
                        ...
                        if config.crls.is_empty() {
                            ...
                            if !has_custom_roots {
                                // Android's trust manager is the source of
                                // truth. `rustls-native-certs` resolves to
                                // openssl_probe on Android and does not read
                                // the Java system trust store.
                                use rustls_platform_verifier::BuilderVerifierExt;
                                config_builder.with_platform_verifier().map_err(|_| {
                                    crate::error::builder(
                                        "Android platform TLS verifier is unavailable",
                                    )
                                })?
                            } else {
