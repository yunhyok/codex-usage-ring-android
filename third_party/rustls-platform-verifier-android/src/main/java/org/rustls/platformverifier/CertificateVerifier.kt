/*
 * Vendored from rustls-platform-verifier v0.7.0, commit
...
internal object CertificateVerifier {
    ...
    private fun verifyCertificateChain(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            val revocationOptions = EnumSet.of(
                PKIXRevocationChecker.Option.SOFT_FAIL,
                PKIXRevocationChecker.Option.ONLY_END_ENTITY
            )
            // Local security adaptation: prefer CRLs only when no stapled OCSP
            // response was supplied. This preserves the upstream OCSP-first
            // behavior for server-provided revocation data.
            if (ocspResponse == null) {
                revocationOptions.add(PKIXRevocationChecker.Option.PREFER_CRLS)
            }
            revocationChecker.options = revocationOptions
        ...
        systemKeystore?.let { loadedSystemKeystore ->
            systemCertificateDirectory?.let { loadedSystemCertificateDirectory ->
                ...
                while (true) {
                    ...
                    // Keep the increment outside the branches so a deleted or malformed anchor
                    // cannot strand this collision scan in an infinite loop.
                    if (anchor is X509Certificate) {
